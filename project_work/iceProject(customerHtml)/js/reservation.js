...
document.querySelectorAll(".reserInfo").forEach((node, idx) => {
  node.addEventListener("change", function () {
    ...
    if (idx == 0) {
      if (
        document.querySelectorAll(".reserInfo")[1].value &&
        !!document.querySelectorAll(".reserInfo")[2].value &&
        !!this.value
      ) {
      ...
      if (
        !!document.querySelectorAll(".reserInfo")[0].value &&
        !!document.querySelectorAll(".reserInfo")[2].value &&
        !!this.value
      ) {
        Raddr.classList.remove("hidden2");
...
PrivacyOk.addEventListener("click", async function () {
  ...
  const reservationData = {
    ...
    addr: `${document.querySelector(".check-addr-post").innerHTML},
                ${document
                  .querySelector(".check-addr-addr")
                  .innerHTML.replaceAll(",", "")},
                ${document
                  .querySelector(".check-addr-deta")
                  .innerHTML.replaceAll(",", "")}`,
    ...
    capacity:
      capacityMap[document.querySelector(".check-capacity").innerHTML] || "",
  ...
  const receviedData = await rest.text();

  if (receviedData === "succcess") {
    Swal.fire({
      title: "예약성공!",
      icon: "success",
      draggable: true,
